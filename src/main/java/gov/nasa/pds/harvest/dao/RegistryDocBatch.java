...
public class RegistryDocBatch {
  ...
  /*
   * hack for PDS-NASA/harvest#127 search the JSON string for any ref_lid_ and add if necessary to
   * index so that it is searchable
   */
  private void updateIndex(ConnectionFactory conFact, String json) {
    ...
    if (-1 < begin_index && alreadyLearned.isEmpty()) {
      try (RestClient client = conFact.createRestClient()) {
        alreadyLearned.addAll(
            client.performRequest(client.createMappingRequest().setIndex(conFact.getIndexName()))
                .fieldNames());
      } catch (Exception e) {
        log.error("Unexpected error (should not have made it here) while getting index "
            + conFact.getIndexName(), e);
      }
    }
    while (-1 < begin_index) {
      ...
      if (!alreadyLearned.contains(name)) {
        try (RestClient client = conFact.createRestClient()) {
          ...
          client.performRequest(client.createMappingRequest().setIndex(conFact.getIndexName())
              .buildUpdateFieldSchema(new_item));
          alreadyLearned.add(name);
        } catch (Exception e) {
          log.error(
              "Unexpected error (should not have made it here) while updating index with " + name,
              e);
        }
      }
